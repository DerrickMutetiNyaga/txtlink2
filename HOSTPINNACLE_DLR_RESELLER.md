...
1. You send SMS via HostPinnacle API â†’ you get back a transaction ID (we store it as `hpTransactionId`).
...
- **In app:** Super-admin â†’ Settings â†’ "Register DLR URL with HostPinnacle" (set `NEXT_PUBLIC_BASE_URL` first). **API:** `POST /api/super-admin/dlr-webhook/register` (super-admin only).
