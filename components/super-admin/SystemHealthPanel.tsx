  ...
  return (
    <div className="relative w-28 h-28 md:w-36 md:h-36">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className={`text-2xl md:text-3xl font-bold ${color}`}>{score}%</span>
...
interface SystemHealthPanelProps {
  compact?: boolean
  /** Pre-loaded report (e.g. from dashboard or queue-status API) */
  initialReport?: SystemHealthReport | null
  /** API URL to fetch health from */
  fetchUrl?: string
}
...
export function SystemHealthPanel({
  compact = false,
  initialReport = null,
  fetchUrl = '/api/super-admin/system-health',
  ...
  const fetchHealth = useCallback(
    async (silent = false) => {
      try {
        ...
        let health: SystemHealthReport | undefined
        if (data.health?.checks) {
          health = data.health
        } else if (data.data?.health?.checks) {
          health = data.data.health
        } else if (data.success && data.checks) {
          health = data as SystemHealthReport
        }
  ...
  useEffect(() => {
    ...
    fetchHealth()
  }, [fetchHealth, initialReport])

  useEffect(() => {
    if (!autoRefresh || initialReport) return
    const interval = setInterval(() => fetchHealth(true), 60000)
    return () => clearInterval(interval)
  }, [autoRefresh, fetchHealth, initialReport])
    ...
    <div className="space-y-6">
      {!compact && (
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h2 className="text-xl md:text-2xl font-semibold text-slate-900">System Health</h2>
            <p className="text-slate-600 mt-1 text-sm">
              Database, SMS gateway, webhooks, queue, and payments.
            </p>
          </div>
      ...
      {loading && !report ? (
        ...
        <>
          <Card className="p-5 md:p-6 border border-slate-200 rounded-2xl shadow-sm bg-white">
            <div className="flex flex-col md:flex-row md:items-center gap-6">
              ...
              <div className="flex-1 space-y-2">
                ...
                <p className="font-medium text-slate-900">{report.summary}</p>
                {!compact && (
                  <p className="text-sm text-slate-600">
                    {report.score >= 90
                      ...
                      : report.score >= 60
                        ...
                        : 'Critical issues detected — review below.'}
                  </p>
                )}
              </div>
              {compact && (
                ...
                <div key={category}>
                  ...
                  <div className="space-y-3">
                    {checks.map((check) => {
                      ...
                      const Icon = cfg.icon
                      return (
                        <Card key={check.id} className={`p-4 border rounded-xl ${cfg.row}`}>
      ...
      {!compact && (
        <Card className="p-4 border-slate-200 bg-slate-50 text-sm text-slate-600">
          <div className="flex items-start gap-2">
            ...
            <p>
              DLR webhooks from HostPinnacle can show degraded while delivery sync stays
              operational — TXTLINK polls HostPinnacle as a backup.
            </p>
          </div>
        </Card>
      )}
